  ...
  private drawGameStatusOverlay(state: LineWarsGameState): void {
    ...
    if (p1 && p2) {
      ...
      if (totalArea > 0) {
        ...
        this.ctx.fillText(`P1: ${((p1Area / totalArea) * 100).toFixed(1)}%`, this.canvas.width / 2 - 100, this.canvas.height / 2 + 40);
        this.ctx.fillStyle = "#ff00ff";
        this.ctx.fillText(`P2: ${((p2Area / totalArea) * 100).toFixed(1)}%`, this.canvas.width / 2 + 100, this.canvas.height / 2 + 40);
      }
    }
  }
